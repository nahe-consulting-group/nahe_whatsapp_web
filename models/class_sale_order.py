...
class SaleOrder(models.Model):
    ...
    def action_sendwslisto(self):
        ...
        if not settings or not settings.sale_message:
            raise ValidationError('No se ha configurado un mensaje para pedidos en WhatsApp.')
        
        for rec in self:
            message = settings.sale_message.replace('#CLIENTE', rec.partner_id.display_name)
